...
//!
//! This drives [`alpenglow::consensus::bench_replay_votes`], which replays a
//! batch of pre-signed votes into a fresh `SlotState`. It deliberately bypasses
//! the consensus loop and signature verification (which would otherwise
//! dominate) so that changes to the per-slot data structures — the
//! `BlockHash`-keyed maps/sets that track notar/notar-fallback stake, parents,
//! and the safe-to-notar bookkeeping — show up in the measurement.
//!
...
///
/// Roughly half notarize one block, a quarter skip, and a quarter cast
/// notar-fallback votes for a competing block — enough to exercise all of the
/// converted per-slot structures (notar / notar-fallback stake maps, the
/// parents map, and the pending/sent safe-to-notar sets).
