...
EXPANSION_RATIO = [(32, 64), (32, 80), (32, 96), (32, 128), (32, 320)]
...
for metric in ['rotor', 'final']:
    ...
    plt.bar(x - 0.5 * w, df_stake_weighted[metric], width=w, label='stake_weighted')
...
for metric in ['rotor', 'final']:
    ...
    plt.bar(x, df_uniform[metric], width=w)
...
df_fa1 = df[df['sampling_strat'] == 'fa1']
df_turbine = df[df['sampling_strat'] == 'turbine']
plt.bar(x - w, df_stake_weighted['safety'], width=w, label='stake_weighted')
...
plt.grid(True, axis='y', linestyle='--', alpha=0.7)
plt.tight_layout()
plt.savefig(f'./data/output/simulations/safety/data_expansion.png', dpi=300)
