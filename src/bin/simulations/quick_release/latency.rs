...
//! Latency simulation for Quick Release MCP, the MCP protocol.
//!
...
use super::{QuickReleaseInstance, QuickReleaseInstanceBuilder, QuickReleaseParams};
...
/// Wrapper type for the Quick Release MCP latency simulation.
///
...
pub struct QuickReleaseLatencySimulation<
    L: SamplingStrategy,
    P: SamplingStrategy,
    R: SamplingStrategy,
> {
...
impl<L, P, R> Protocol for QuickReleaseLatencySimulation<L, P, R>
where
    L: SamplingStrategy,
    P: SamplingStrategy,
    R: SamplingStrategy,
{
    ...
    type Params = QuickReleaseParams;
    type Instance = QuickReleaseInstance;
    type Builder = QuickReleaseInstanceBuilder<L, P, R>;
}

/// Stages of the Quick Release MCP latency simulation.
...
/// Events that can occur at each validator during the Quick Release MCP latency simulation.
#[derive(Clone, Copy, Debug, PartialEq, Eq, PartialOrd, Ord, Hash)]
...
impl Event for LatencyEvent {
    ...
    fn dependencies(&self, _params: &QuickReleaseParams) -> Vec<Self> {
        match self {
            Self::Propose => vec![],
