...
use crate::consensus::{
    Cert, EpochInfo, FinalVote, NotarFallbackVote, NotarVote, SkipFallbackVote, SkipVote, Vote,
};
use crate::crypto::merkle::BlockHash;
use crate::{BlockId, Slot, Stake, ValidatorIndex};
...
pub(super) struct SlotState {
    ...
    slot: Slot,
    /// Validator set for this slot's epoch (`epoch(slot)`).
    ///
    /// Pinning the set here is what makes vote/cert validation epoch-relative:
    /// all stake totals, quorum thresholds, and certificate bitmaps below are
    /// computed against the set of the epoch this slot belongs to.
    pub(super) epoch_info: Arc<EpochInfo>,
    /// Our own index within this slot's epoch, or `None` if we are not a member.
    pub(super) own_index: Option<ValidatorIndex>,
}
...
impl SlotState {
    ...
    pub(super) fn new(
        ...
        own_index: Option<ValidatorIndex>,
    ) -> Self {
        Self {
    ...
    fn count_notar_stake(
        ...
        *notar_stake += stake;
        ...
        if !self.sent_safe_to_skip
            ...
            && self
                .own_index
                .is_some_and(|i| self.votes.notar[i.as_usize()].is_some())
        {
    ...
    fn count_skip_stake(&mut self, slot: Slot, stake: Stake, fallback: bool) -> SlotStateOutputs {
        ...
        if self.epoch_info.is_quorum(total_skip_stake) && self.certificates.skip.is_none() {
            ...
            let cert =
                SkipCert::new_unchecked(&skip_votes, &sf_votes, self.epoch_info.validators());
    ...
    fn check_safe_to_notar(&mut self, block_hash: BlockHash) -> SafeToNotarStatus {
        ...
        // check own vote (a non-member of this epoch has no own vote)
        let skip = self
            .own_index
            .and_then(|i| self.votes.skip[i.as_usize()].as_ref());
        let notar = self
            .own_index
            .and_then(|i| self.votes.notar[i.as_usize()].as_ref());
...
mod tests {
    ...
    /// Our own index in these single-epoch tests (we are validator 0).
    const OWN: Option<ValidatorIndex> = Some(ValidatorIndex::new(0));

    fn wrap_epoch_info(epoch_info: EpochInfo) -> Arc<EpochInfo> {
        Arc::new(epoch_info)
    }
    ...
    fn add_cert() {
        ...
        let mut slot_state = SlotState::new(slot, epoch_info.clone(), OWN);
    ...
    fn add_vote() {
        ...
        for (i, sk) in sks.iter().enumerate() {
            ...
            let voter_stake = epoch_info.validator(ValidatorIndex::new(i as u64)).stake;
    ...
    fn safe_to_notar() {
        ...
        let voter_stake = epoch_info.validator(ValidatorIndex::new(1)).stake;
        ...
        let voter_stake = epoch_info.validator(ValidatorIndex::new(0)).stake;
        let (certs, events, blocks) = slot_state.add_vote(vote.clone(), voter_stake);
        assert!(certs.is_empty());
