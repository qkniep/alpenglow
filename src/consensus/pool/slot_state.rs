...
impl SlotState {
    ...
    pub fn notify_parent_known(&mut self, hash: BlockHash) {
        ...
        if self.parents.contains_key(&hash) {
            warn!(
                "parent of block {hash} in slot {} was alredy known",
                self.slot
            );
            return;
        }
    ...
    pub fn notify_parent_certified(
        &mut self,
        hash: BlockHash,
    ) -> Option<Either<VotorEvent, (Slot, BlockHash)>> {
...
mod tests {
    ...
    fn add_cert() {
        ...
        let votes: Vec<_> = sks
            ...
            .enumerate()
            .map(|(i, sk)| {
                Vote::new_notar(Slot::new(1), BlockHash::genesis(), sk, i as ValidatorId)
            })
            .collect();
    ...
    fn add_vote() {
        ...
        for (i, sk) in sks.iter().enumerate() {
            let vote = Vote::new_notar(Slot::new(1), BlockHash::new_random(), sk, i as ValidatorId);
            ...
            assert_eq!(
                slot_state.voted_stakes.notar.get(&BlockHash::new_random()),
                Some(&((i + 1) as Stake))
            );
    ...
    fn safe_to_notar() {
        ...
        slot_state.notify_parent_known(BlockHash::new_random());
        slot_state.notify_parent_certified(BlockHash::new_random());

        // 33% notar alone has no effect
        let vote = Vote::new_notar(Slot::new(1), BlockHash::new_random(), &sks[1], 1);
        ...
        match events[0] {
            VotorEvent::SafeToNotar(s, h) => {
                ...
                assert_eq!(h, BlockHash::new_random());
            }
            _ => unreachable!(),
        }
    }
