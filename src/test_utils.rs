...
pub fn create_random_block(slot: Slot, num_slices: usize) -> Vec<Slice> {
    ...
    for slice_index in final_slice_index.until() {
        ...
        // Reserve space in the last slice for the execution state hash suffix.
        let reserve = if is_last {
            crate::types::slice::STATE_HASH_SIZE
        } else {
            0
        };
        let mut payload = create_random_slice_payload_valid_txs(parent, reserve);
        if is_last {
            // Append a dummy (all-zero) execution state hash.
            payload
                .data
                .extend_from_slice(&[0u8; crate::types::slice::STATE_HASH_SIZE]);
        }
...
fn create_random_slice_payload_valid_txs(
    parent: Option<BlockId>,
    reserve_suffix: usize,
) -> SlicePayload {
    ...
    // Reserve `reserve_suffix` bytes at the end (e.g. for the state hash on the last slice).
    let num_txs = {
        let serialized_tx_len = tx.len() + 8; // 8 bytes for wincode Vec<u8> length prefix
        let capacity = MAX_DATA_PER_SLICE.saturating_sub(reserve_suffix);
        // Use fewer transactions if we need to leave room for the suffix.
        let possible = (capacity.saturating_sub(8 + 1)) / serialized_tx_len; // 8 = outer len, 1 = parent
        possible.min(NUM_TXS_PER_SLICE)
    };
    ...
    assert!(
        payload.len() + reserve_suffix <= MAX_DATA_PER_SLICE,
        "payload.len()={} + reserve={} > {}",
        payload.len(),
        reserve_suffix,
        MAX_DATA_PER_SLICE
